...
class Attention(nn.Module):
    ...
    def __init__(self, encoder_hidden_size, decoder_hidden_size, use_heightmap):
...
class Pointer(nn.Module):
    ...
    def __init__(self, encoder_hidden_size, decoder_hidden_size, use_heightmap, num_layers=1, dropout=0.2):
        ...
        self.encoder_attn = Attention( encoder_hidden_size, decoder_hidden_size, use_heightmap)
...
class DRL(nn.Module):
    def __init__(self, static_size, dynamic_size, encoder_hidden_size, decoder_hidden_size, 
                use_cuda, input_type, allow_rot, container_width, block_dim, reward_type, 
                use_heightmap, only_heightmap, packing_strategy,
                containers,
                update_fn, mask_fn, num_layers=1, dropout=0.):
        ...
        self.pointer = Pointer(encoder_hidden_size, decoder_hidden_size, use_heightmap, num_layers, dropout)
        ...
        self.only_heightmap = only_heightmap
...
def train_pack(args):
    ...
    containers = [tools.Container(container_size, args.total_blocks_num, args.reward_type, initial_container_size, packing_strategy=args.packing_strategy)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_blocks_num', default=6, type=int)

    parser.add_argument('--only_heightmap', default=False, type=str2bool)
    parser.add_argument('--packing_strategy', default=False, type=str2bool)
    ...
    if args.task == 'pack':
        ...
        print('Heightmap:        %s' % args.use_heightmap)
        print('Only Height:      %s' % args.only_heightmap)
        print('Target Container: %s' % args.container_width)
        print('Init container:   %s' % args.initial_container_width)
        print('Use final pack:   %s' % args.packing_strategy)
